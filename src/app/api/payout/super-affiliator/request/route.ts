...
export async function POST(request: Request) {
  try {
    ...
    if (!session?.user || session.user.role !== "super-affiliator") {
    ...
    if (!superAffiliator) {
      return NextResponse.json({ error: "Super affiliator profile not found" }, { status: 404 });
    }

    if (superAffiliator.tokenBalance < payload.amount) {
    ...
    const existingPending = await prisma.payoutRequest.findFirst({
      where: { superAffiliatorId: superAffiliator.id, status: "pending" },
    });
    ...
    console.error("Failed to submit super affiliator payout request", error);
